import tensorflow as tf  # pylint ignore
import numpy as np
...
class ModelBasedPolicy(object):
    ...
    def __init__(self,
                 ...
                 nn_layers=1):
        self._action_space = env.action_space  # my addition
    ...
    def _setup_placeholders(self):
        ...
        state_ph = tf.placeholder(
            dtype=tf.float32, shape=[None, self._state_dim], name='state')
        action_ph = tf.placeholder(
            dtype=tf.float32, shape=[None, self._action_dim], name='action')
        next_state_ph = tf.placeholder(
            dtype=tf.float32, shape=[None, self._state_dim], name='next_state')
    ...
    def _dynamics_func(self, state, action, reuse):
        ...
        """
        # PROBLEM 1
        # 1) DONE - normalize state and action according to initdataset
        # 2) DONE - concatenate state action pair
        # check this concat later
        # print(self._init_dataset.state_mean.shape)
        normalized_state = utils.normalize(
            state, self._init_dataset.state_mean, self._init_dataset.state_std)
        ...
        normalized_Delta = utils.build_mlp(state_action, self._state_dim,
                                           scope='dynamics_model', n_layers=self._nn_layers, reuse=reuse)
        # 3) DONE - unnormalize Delta
        Delta = utils.unnormalize(
            normalized_Delta, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)

        # 4) DONE - add Delta to state to get next state
        next_state_pred = Delta + state
    ...
    def _setup_training(self, state_ph, next_state_ph, next_state_pred):
        ...
        predicted_Delta = next_state_pred - state_ph
        # b) TODO - normalize
        actual_Delta = utils.normalize(
            actual_Delta, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
        predicted_Delta = utils.normalize(
            predicted_Delta, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
        # c)
    ...
    def _setup_action_selection(self, state_ph):
        ...
        # PROBLEM 2
        # YOUR CODE HERE
        # a H x act_dim x N_samples tensor
        ...
        actions = tf.random_uniform(
            (n, h, a), minval=self._action_space_low, maxval=self._action_space_high)
        states = tf.stack([state_ph[0]] * n)
        costs = tf.zeros(n)
        for t in range(h):
            actions_per_sample = actions[:, t, :]
            next_states = self._dynamics_func(
                states, actions_per_sample, True)
            costs += self._cost_fn(states, actions_per_sample, next_states)
            states = next_states
        lowest_cost_idx = tf.argmin(costs)
        best_actions = actions[lowest_cost_idx]
    ...
    def train_step(self, states, actions, next_states):
        ...
        loss, _ = self._sess.run([self._loss, self._optimizer], feed_dict={
            self._state_ph: states, self._action_ph: actions, self._next_state_ph: next_states})
    ...
    def predict(self, state, action):
        ...
        next_state_pred = self._sess.run(self._next_state_pred, feed_dict={
            self._state_ph: [state], self._action_ph: [action]}).reshape(-1)
