...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        output_placeholder = tf.layers.dense(
            x, units=output_size, activation=output_activation)
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_CODE_HERE
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, "policy_net",
                                     self.n_layers, self.size, activation=tf.tanh, output_activation=None)
            return sy_logits_na
        else:
            # YOUR_CODE_HERE
            sy_mean = build_mlp(sy_ob_no, self.ac_dim, "policy_net",
                                self.n_layers, self.size, activation=tf.tanh, output_activation=None)
            sy_logstd = tf.get_variable(
                'log_std', initializer=0.5*np.ones(self.ac_dim, dtype=np.float32))
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # YOUR_CODE_HERE
            # first - tried to take the argmax - WRONG!
            # second - samples from multinomial dist
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        """
        def gaussian_log_likelihood(act, mu, log_std):
            return tf.reduce_sum(-0.5*((act-mu)**2/tf.exp(log_std)+2*log_std+np.log(2*np.pi)), axis=1)

        if self.discrete:
            sy_logits_na = policy_parameters
            # YOUR_CODE_HERE
            # sy_logprob_n = tf.nn.log_softmax(sy_logits_na)
            # first try - used softmax function to get log likelihoods WRONG
            # second try - used cross entropy softmax loss
            sy_logprob_n = - tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=sy_ac_na, logits=sy_logits_na)
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_CODE_HERE
            sy_logprob_n = gaussian_log_likelihood(
                sy_ac_na, sy_mean, sy_logstd)
        return sy_logprob_n
    ...
    def build_computation_graph(self):
        ...
        self.pi_loss = -tf.reduce_mean(self.sy_logprob_n *
                                       self.sy_adv_n)  # YOUR CODE HERE

        self.update_op = tf.train.AdamOptimizer(
            self.learning_rate).minimize(self.pi_loss)
        ...
        if self.nn_baseline:
            ...
            baseline_loss = .5 * \
                tf.reduce_mean(tf.squared_difference(
                    self.sy_target_n, self.baseline_prediction))
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            ...
            ac = self.sess.run(self.sy_sampled_ac, feed_dict={
                               self.sy_ob_no: ob.reshape(1, self.ob_dim)})
            ac = ac[0]
            acs.append(ac)
    ...
    def sum_of_rewards(self, re_n):
        ...
        def compute_rtg(rews, gamma):
            # O(T**2) where T = len(rews)
            # but fast due to vectorization
        ...
        def compute_r(rews, gamma):
            # O(T) where T = len(rews)
            sum_r = sum([gamma ** t * r for t, r in enumerate(rews)])
            return [sum_r for _ in rews]

        def discounted_vals(re_i, gamma, reward_to_go):
            return compute_rtg(re_i, gamma) if reward_to_go else compute_r(re_i, gamma)

        def q_values(re_n, gamma, reward_to_go):
            # get list of lists
            q_n = [discounted_vals(re_i, gamma, reward_to_go)
                   for re_i in re_n]
            q_n = np.hstack(q_n)
            return q_n

        q_n = q_values(re_n, self.gamma, self.reward_to_go)
    ...
    def compute_advantage(self, ob_no, q_n):
        ...
        if self.nn_baseline:
            ...
            #print('b and q means:', b_n.mean(), q_n.mean())
            #print('b and q stds:', b_n.std(), q_n.std())
            # YOUR CODE HERE
            adv_n = q_n - b_n
        else:
            adv_n = q_n.copy()
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.nn_baseline:
            ...
            adv_n = self.compute_advantage(ob_no, q_n)
            target_n = q_n
            prediction = q_n - adv_n

            self.sess.run(self.baseline_update_op,
                          feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: target_n, self.baseline_prediction: prediction})
