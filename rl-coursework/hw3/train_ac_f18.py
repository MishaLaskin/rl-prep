...
class Agent(object):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE - DONE
        q_n_sa = re_n + \
            (1-terminal_n) * self.sess.run(self.critic_prediction,
                                           {self.sy_ob_no: next_ob_no})
        v_n_s = self.sess.run(self.critic_prediction, {
                              self.sy_ob_no: ob_no})
        adv_n = q_n_sa - v_n_s
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            bootstrap_targets = re_n + (1-terminal_n) * self.sess.run(
                self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})

            for _ in range(self.num_grad_steps_per_target_update):

                _ = self.sess.run(self.critic_update_op, feed_dict={
                    self.sy_target_n: bootstrap_targets, self.sy_ob_no: ob_no})
