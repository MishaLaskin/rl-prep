...
class QLearner(object):
    ...
    def __init__(
        ...
        def collect_vars(x):
            return tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope=x)

        def create_mask(x, dims):
            return tf.one_hot(x, dims)

        def get_masked_vals(all_vals, mask_vec, mask_dim):
            return tf.reduce_sum(create_mask(mask_vec, mask_dim)*all_vals, axis=1)

        # current q value q(s,a) - a matrix of dimension (N,act_dim)
        pred_q = q_func(obs_t_float, self.num_actions, "pred_q", reuse=False)
        self.pred_q = pred_q

        # q values only for actions taken - vector of dimension (N,)
        pred_y = get_masked_vals(pred_q, self.act_t_ph, self.num_actions)

        # target q value q(s',a') - a matrix of dimension (N,act_dim)
        target_q = q_func(obs_tp1_float, self.num_actions,
                          "target_q", reuse=False)

        if double_q:
            # double dqn uses online actions instead of taken actions for q(s',a')
            # a (N,act_dim) matrix
            online_q = q_func(obs_tp1_float, self.num_actions,
                              "online_q", reuse=False)
            # maximum q value action
            online_actions = tf.argmax(online_q, axis=1)
            # targets
            max_target_q = get_masked_vals(
                target_q, online_actions, self.num_actions)
        else:
            # get max_a' q(s',a')
            max_target_q = tf.reduce_max(target_q, axis=1)

        # bellman error (q(s,a)-[r(a,a) + gamma (1-terminal) max_a' q(s',a')])**2 - or use huber loss
        target_y = self.rew_t_ph + gamma*(1-self.done_mask_ph)*max_target_q

        # define the error
        error_vec = pred_y - target_y
        self.total_error = huber_loss(error_vec)

        # lastly, get q func vars
        q_func_vars = collect_vars('pred_q')
        target_q_func_vars = collect_vars('target_q')
    ...
    def step_env(self):
        ...
        if np.random.randn() > epsilon and self.model_initialized:
            # take action given by q
            action = self.session.run(tf.argmax(self.pred_q, axis=1), feed_dict={
                self.obs_t_ph: [obs]})
            action = action[0]
        else:
